...
public interface CommonuserRepository extends JpaRepository<Commonuser, Integer>{
	...
	@Query(value="SELECT * FROM commonuser WHERE security_question=:question AND security_answer=:answer",nativeQuery=true)
	public Commonuser findByQuestionAndAnswer(@Param("question") String question,@Param("answer") String anwser);
}
